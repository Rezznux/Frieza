"""frieza - unified CLI for APK Intercept Kit.
...
def cmd_gadget_patch(args: argparse.Namespace) -> int:
    from apk_intercept.gadget_repack import patch_decoded_tree_for_gadget

    result = patch_decoded_tree_for_gadget(args.decoded_dir, library_name=args.library_name)
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--version", action="version", version="frieza 0.3.0")
    parser.add_argument(
        "--workspace",
        metavar="DIR",
        help="Workspace root for reports, repacks, logs, and session state (default: FRIEZA_HOME or platform data dir).",
    )
    parser.add_argument(
        "--session",
        metavar="DIR",
    ...
    p.add_argument("--report", required=True, metavar="PATH", help="Static report JSON from 'frieza static'")
    ...
    p = sub.add_parser(
        "gadget-patch",
        help="Patch a decoded apktool tree to load an embedded Frida Gadget at startup",
        description=(
            "Finds the best injection point (Application.onCreate preferred, launcher "
            "Activity.onCreate fallback) and inserts a System.loadLibrary() smali snippet. "
            "Run after 'apktool d' and before 'frieza repack'."
        ),
    )
    p.add_argument("--decoded-dir", required=True, metavar="DIR", help="Path to apktool decoded directory")
    p.add_argument("--library-name", default="gadget", metavar="NAME", help="Library name passed to System.loadLibrary (default: gadget)")
