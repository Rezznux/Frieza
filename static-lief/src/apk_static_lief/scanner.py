...
def analyze_apk_full(
    ...
    with tempfile.TemporaryDirectory(prefix="apk-decompile-") as decomp_dir:
        ...
        if vulnscan:
            ...
            if decompile_ok:
                # Scope scan to the app's own package to avoid scanning thousands
                # of third-party library files (Firebase, OkHttp, Bouncy Castle…).
                # JADX outputs to <decomp_dir>/sources/<pkg/path/>, so convert the
                # dot-separated package name to a path and use it if it exists.
                scan_dir = decomp_dir
                pkg = summary.get("manifest", {}).get("package_name")
                if pkg:
                    import os as _os
                    pkg_path = _os.path.join(decomp_dir, "sources", pkg.replace(".", _os.sep))
                    if _os.path.isdir(pkg_path):
                        scan_dir = pkg_path
                vuln_result = (
                    run_semgrep(scan_dir, timeout=600)
