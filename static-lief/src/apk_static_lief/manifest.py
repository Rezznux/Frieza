...
def _parse_binary_manifest(apk_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # Permissions may be stored as UTF-16LE (Android binary XML) or ASCII.
        _PERM_STR_RE = re.compile(r"android\.permission\.[A-Z_]{3,60}")
        for text in (raw.decode("utf-16-le", errors="ignore"), raw.decode("ascii", errors="ignore")):
            for m in _PERM_STR_RE.finditer(text):
                perm = m.group(0)
                if perm not in result["permissions"]:
                    result["permissions"].append(perm)
            if result["permissions"]:
                break
        # The app's own package name appears as an exact null-terminated string in
        # the binary manifest (it IS the package attribute value). Third-party SDK
        # packages only appear as prefixes of longer class/authority strings.
        # Strategy: find non-framework candidates that appear standalone (followed by
        # \x00) — the one with the most such occurrences is the app package.
        _FRAMEWORK_PREFIXES = (
            "android.", "androidx.", "com.google.", "com.android.", "kotlin.",
            "java.", "javax.", "org.jetbrains.", "org.apache.", "io.grpc.",
            "dalvik.", "libcore.", "sun.", "com.squareup.", "okhttp3.",
        )
        _PKG_PAT = re.compile(r"[a-z][a-z0-9]{1,}(?:\.[a-z][a-z0-9]{1,}){2,}")
        for text in (raw.decode("utf-16-le", errors="ignore"), raw.decode("ascii", errors="ignore")):
            candidates = {
                c for c in _PKG_PAT.findall(text)
                if not any(c.startswith(p) for p in _FRAMEWORK_PREFIXES)
            }
            # Count standalone occurrences (exact string followed by null or space)
            scored = {
                c: text.count(c + "\x00") + text.count(c + " ")
                for c in candidates
            }
            best = max(scored, key=lambda k: scored[k], default=None)
            if best and scored[best] > 0:
                result["package_name"] = best
                break
    except Exception:
        pass
    return result
